  ...
  <div class="admin-dashboard">
    ...
    <div class="partner-section">
      <div class="partner-section-top">
        <h3 class="table-title">PARTNERE OG LEADS</h3>
      ...
      <template v-if="!isLoading && activeTab === 'partners'">
        ...
        >
          <div class="partner-info">
            ...
            <p v-if="person.companyName">
              Virksomhed: {{ person.companyName }}
            </p>

            <small
              v-if="getAssignedCourseCount(person._id) > 0"
              class="assigned-count"
              @click="openAssignModal(person)"
            >
              {{ getAssignedCourseCount(person._id) }} kursus/kurser tildelt ·
              rediger
            </small>
          </div>

          <span class="status" :class="statusClassFor(person.status)">
        ...
        <div v-if="filteredKlarTilKursus.length === 0" class="empty-state">
      ...
      <template v-if="!isLoading && activeTab === 'leads'">
        ...
        >
          ...
          <span class="status waiting"> Survey sendt </span>
        ...
        <div v-if="filteredIgangvaerende.length === 0" class="empty-state">
    ...
    <!-- TILDEL KURSER MODAL -->
    <div v-if="showAssignModal && selectedPartner" class="assign-modal-overlay">
      <div class="assign-modal">
        <header class="assign-modal-header">
          <div>
            <h2>
              Tildel kurser til
              {{ selectedPartner.contactPerson || selectedPartner.email }}
            </h2>
            <p>Vælg de kurser som denne kunde skal gennemgå.</p>
          </div>

          <button
            class="assign-modal-close"
            type="button"
        ...
        <div class="assign-course-list">
          <button
            ...
            :class="{
              'assign-course-card-active': selectedCourseIds.includes(
                course._id,
              ),
            }"
            type="button"
        ...
        <p v-if="assignError" class="lead-error assign-error">
          {{ assignError }}
        </p>

        <footer class="assign-modal-footer">
          <button
            class="assign-btn assign-btn-light"
            type="button"
            @click="closeAssignModal"
          >
            Annuller
          ...
          <button
            class="assign-btn assign-btn-dark"
            type="button"
    ...
    <!-- SURVEY SVAR MODAL -->
    <div
      v-if="showSurveyModal && selectedSurveyPerson"
    ...
    >
      <div class="survey-modal">
        ...
        <div class="survey-answer-list">
          <div v-if="loadingSurvey" class="empty-state">
            Henter survey-svar...
          </div>

          <template v-else>
            <div v-if="surveyAnswers.length === 0" class="empty-state">
            ...
            >
              <strong>{{ formatQuestionLabel(answer.questionId) }}</strong>
    ...
    <div
      v-if="showDeleteModal && pendingDeletePerson"
      class="assign-modal-overlay"
    ...
    >
      <div class="confirm-modal">
        <header class="assign-modal-header">
          <div>
            ...
            <p>
              Du er ved at slette
              <strong>{{
                pendingDeletePerson.contactPerson || pendingDeletePerson.email
              }}</strong
              >. Alle kursustildelinger og survey-svar fjernes også. Handlingen
              kan ikke fortrydes.
            </p>
          </div>
        ...
        <p v-if="deleteError" class="lead-error assign-error">
          {{ deleteError }}
        </p>
        ...
        <footer class="assign-modal-footer">
          ...
          <button
            class="assign-btn assign-btn-danger"
            type="button"
            :disabled="!!deletingForId"
            @click="confirmDelete"
          >
...
import { Users, BookOpen, TrendingUp, Trash2 } from "lucide-vue-next";
...
function formatQuestionLabel(questionId) {
  const labels = {
    ...
    expected_volume: "Forventet volumen",
  };

  return labels[questionId] || String(questionId ?? "").replaceAll("_", " ");
}

function formatAnswer(answer) {
  if (Array.isArray(answer)) {
    return answer.map(capitalizeFirstLetter).join(", ");
  }
...
function handleLeadCreated(newLeads) {
  ...
  const filteredNewLeads = leadsToAdd.filter((lead) => {
    return !customers.value.some((existing) => existing._id === lead._id);
  });
...
async function openAssignModal(person) {
  ...
  try {
    ...
    if (res.ok) {
      const data = await res.json();
      const courseIds = (data.assignments || []).map((a) => String(a.courseId));
...
async function saveAssignedCourses() {
  ...
  try {
      ...
      ...toAdd.map((courseId) =>
        fetch(`${API_URL}/admin/assign-course`, {
          ...
          if (!res.ok && res.status !== 409) {
            ...
            throw new Error(body.message || "Kunne ikke tildele kursus.");
          }
        }),
      ),
      ...toRemove.map((courseId) =>
        fetch(`${API_URL}/admin/customers/${partnerId}/courses/${courseId}`, {
          method: "DELETE",
          credentials: "include",
        }).then(async (res) => {
          if (!res.ok && res.status !== 404) {
            const body = await res.json().catch(() => ({}));
            throw new Error(body.message || "Kunne ikke fjerne kursus.");
          }
        }),
      ),
    ]);
    ...
    const partnerLabel =
      selectedPartner.value.contactPerson || selectedPartner.value.email;
    ...
    if (wasFirstAssignment) {
      ...
      try {
        ...
        loadError.value =
          err.message || "Kurserne blev tildelt, men mailen kunne ikke sendes.";
      }
    } else {
      showSuccess(`${next.size} kursus/kurser tildelt til ${partnerLabel}.`);
    }
  } catch (err) {
...
async function confirmDelete() {
  ...
  try {
    const res = await fetch(`${API_URL}/admin/customers/${person._id}`, {
      method: "DELETE",
      credentials: "include",
    });
    ...
    customers.value = customers.value.filter((c) => c._id !== person._id);
...
async function resendCoursesFor(person) {
  ...
  try {
    ...
    showSuccess(`Kursusmail gensendt til ${person.email}.`);
  } catch (err) {
