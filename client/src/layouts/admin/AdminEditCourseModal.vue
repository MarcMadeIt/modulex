<script setup>
    ...
    const courseId = computed(() => {
        return (
            ...
            props.courseData.raw?._id ||
            props.courseData.raw?.id ||
            props.courseData.course?._id ||
            props.courseData.course?.id
        );
    });

    const baseCourse = computed(() => {
        return props.courseData.raw || props.courseData.course || props.courseData;
    });
    ...
    async function apiFetch(path, options = {}) {
        ...
        try {
            ...
            if (error.name === "AbortError") {
                throw new Error("API-kaldet tog for lang tid. Tjek om backend kører.");
            }
    ...
    async function loadCourseData() {
        ...
        try {
            if (!courseId.value) {
                throw new Error("CourseId mangler. Dashboard sender ikke korrekt kursus videre.");
            }

            courseTitle.value = isDuplicateMode.value
                ? `${baseCourse.value.title || ""} (Kopi)`
                : baseCourse.value.title || "";

            courseDescription.value = baseCourse.value.description || "";

            console.log("EDIT COURSE DATA:", props.courseData);
            console.log("EDIT COURSE ID:", courseId.value);

            const data = await apiFetch(`/admin/courses/${courseId.value}/modules`);

            const apiModules = Array.isArray(data)
                ? data
                : Array.isArray(data.modules)
                    ? data.modules
                    : Array.isArray(data.course?.modules)
                        ? data.course.modules
                        : [];
            ...
            submitError.value = error.message || "Kunne ikke hente kursusets moduler.";
            modules.value = [];
        } finally {
    ...
    async function loadLibrary() {
        ...
        try {
            ...
            contentLibrary.value = Array.isArray(data.content)
                ? data.content
                : Array.isArray(data)
                    ? data
                    : [];
    ...
    function mapApiModuleToModule(module) {
        const materials = module.materials || module.contents || [];

        const materialWithDuration = materials.find((material) => {
            return material.expectedDuration !== undefined || material.duration !== undefined;
        });

        const durationFromMaterial =
            materialWithDuration?.expectedDuration ?? materialWithDuration?.duration ?? "";

        const durationValue =
            module.duration ?? module.expectedDuration ?? durationFromMaterial ?? "";

        return {
            localId: makeLocalId(),
            _id: module._id || module.id || null,
            title: module.title || "",
            description: module.description || "",
            duration: formatDurationForInput(durationValue),
            materials: materials.map(mapApiMaterial),
        };
    }

    function mapApiMaterial(material) {
        const isVideo = material.type === "youtube" || material.type === "video";
        const url = material.url || material.fileUrl || "";

        return {
            localId: makeLocalId(),
            source: material.contentId ? "library" : "manual",
            contentId: material.contentId || null,
            type: isVideo ? "video" : "pdf",
            title: material.title || "",
            youtubeUrl: isVideo ? url : "",
            file: null,
            fileUrl: isVideo ? "" : url,
            size: material.size || "",
        };
    ...
    function removeModule(moduleId) {
        const moduleToRemove = modules.value.find((module) => {
            return module.localId === moduleId;
        });

        if (moduleToRemove?._id && !isDuplicateMode.value) {
            deletedModuleIds.value.push(moduleToRemove._id);
        }
    ...
    function removeMaterial(module, materialId) {
        module.materials = module.materials.filter((material) => {
            return material.localId !== materialId;
        });
    }
    ...
    function getYoutubeEmbedUrl(url) {
        ...
        if (urlText.includes("youtube.com/embed/")) {
            return urlText;
        }

        let videoId = "";

        if (urlText.includes("youtube.com/watch?v=")) {
            videoId = urlText.split("v=")[1]?.split("&")[0];
        } else if (urlText.includes("youtu.be/")) {
            videoId = urlText.split("youtu.be/")[1]?.split("?")[0];
        } else if (urlText.includes("youtube.com/shorts/")) {
            videoId = urlText.split("shorts/")[1]?.split("?")[0];
        }
    ...
    function isMaterialValid(material) {
        const hasTitle = toText(material.title).trim() !== "";
        ...
        if (material.type === "pdf") {
            return Boolean(material.file || material.fileUrl);
        }
    ...
    const filteredLibrary = computed(() => {
        ...
        return contentLibrary.value.filter((item) => {
            return (
                toText(item.title).toLowerCase().includes(query) ||
                toText(item.description).toLowerCase().includes(query) ||
                toText(item.category).toLowerCase().includes(query)
            );
        });
    });

    const isFormValid = computed(() => {
        if (!toText(courseTitle.value).trim()) return false;
        if (!toText(courseDescription.value).trim()) return false;
        if (modules.value.length === 0) return false;

        return modules.value.every((module) => {
            const moduleTitle = toText(module.title).trim();
            const moduleDuration = toText(module.duration).trim();

            if (!moduleTitle) return false;
            if (!moduleDuration) return false;
    ...
    function buildCoursePayload() {
        return {
            ...
            modules: modules.value.map((module, moduleIndex) => {
                return buildModulePayload(module, moduleIndex);
            }),
        };
    }

    function buildModulePayload(module, moduleIndex) {
        const expectedDuration = parseDurationMinutes(module.duration);

        return {
            title: toText(module.title).trim(),
            description: module.description || "",
            order: moduleIndex + 1,
            ...
            materials: module.materials.map((material) => {
                ...
                if (material.type === "video") {
                    ...
                    return {
                        type: "youtube",
                        title: toText(material.title).trim(),
                        url,
                        expectedDuration,
                        contentId,
                    };
    ...
    async function saveCourse() {
        ...
        try {
            ...
            submitError.value = error.message || "Kunne ikke gemme kurset.";
        } finally {
    ...
    async function updateExistingCourse() {
        await apiFetch(`/admin/courses/${courseId.value}`, {
            method: "PUT",
            body: JSON.stringify({
                title: toText(courseTitle.value).trim(),
                description: toText(courseDescription.value).trim(),
            }),
        });

        for (const moduleId of deletedModuleIds.value) {
            await apiFetch(`/admin/courses/${courseId.value}/modules/${moduleId}`, {
                method: "DELETE",
            });
        }
        ...
        for (let index = 0; index < modules.value.length; index++) {
            ...
            const payload = buildModulePayload(module, index);

            if (module._id) {
                await apiFetch(`/admin/courses/${courseId.value}/modules/${module._id}`, {
                    method: "PUT",
                    body: JSON.stringify(payload),
                });
            } else {
                await apiFetch(`/admin/courses/${courseId.value}/modules`, {
                    method: "POST",
                    body: JSON.stringify(payload),
                });
            }
    ...
    async function createDuplicateCourse() {
    ...
    <div class="modal-overlay" @click.self="closeModal">
        <div class="modal">
            <header class="modal-header">
                <div>
                    <h1>
                        {{ isDuplicateMode ? "Duplikér kursus" : "Redigér kursus" }}
                    </h1>
              ...
              isDuplicateMode
                ...
                : "Redigér kursusindhold og moduler."
                        }}
                    </p>
                </div>
            ...
            <div v-if="isLoading" class="modal-body">
                <div class="library-state">Henter kursus og moduler...</div>
            </div>

            <div v-else class="modal-body">
                <div v-if="submitError && modules.length === 0" class="submit-error">
                    {{ submitError }}

                    <button class="btn btn-light retry-btn" type="button" @click="loadCourseData">
                        Prøv igen
                    </button>
                </div>
                ...
                <template v-else>
                    <section class="form-section">
                        ...
                        <div class="form-group">
                            <label class="input-label">
                                Kursus titel <span class="required">*</span>
                            </label>
                        ...
                        <div class="form-group">
                            <label class="input-label">
                                Beskrivelse <span class="required">*</span>
                            </label>
                    ...
                    <section class="form-section">
                        ...
                        <article v-for="(module, moduleIndex) in modules"
                                 :key="module.localId"
                                 class="module-card">
                            ...
                            <div class="form-grid">
                                <div class="form-group">
                                    <label class="input-label">
                                        Titel på modul <span class="required">*</span>
                                    </label>
                                ...
                                <div class="form-group">
                                    <label class="input-label">
                                        Varighed <span class="required">*</span>
                                    </label>

                                    <input v-model="module.duration"
                                           class="input"
                                           type="text"
                            ...
                            <div class="material-section">
                                ...
                                <div v-if="openLibraryPickerFor === module.localId"
                                     class="library-picker">
                                    <div class="library-picker-header">
                                        <div class="library-search">
                                            <Search :size="14" />

                                            <input v-model="librarySearch"
                                                   type="text"
                                    ...
                                    <div v-else-if="libraryStatus === 'error'"
                                         class="library-state error">
                                        {{ libraryError }}
                                    ...
                                    <div v-else-if="filteredLibrary.length === 0"
                                         class="library-state">
                                        Ingen content matcher søgningen.
                                    </div>

                                    <ul v-else class="library-list">
                                        <li v-for="item in filteredLibrary"
                                            :key="item._id"
                                            class="library-item"
                                ...
                                <div v-if="module.materials.length === 0"
                                     class="material-empty-state">
                                    <BookOpen :size="20" />
                                    <strong>Ingen materialer endnu</strong>
                                    <p>Tilføj mindst ét materiale for at gemme modulet.</p>
                                </div>

                                <div v-for="(material, materialIndex) in module.materials"
                                    ...
                                    <div class="material-card-top">
                                        <div class="material-card-meta">
                                            ...
                                            <span class="material-source-badge"
                          ...
                          material.source === "library"
                            ...
                            : "Manuelt"
                                    ...
                                    <div class="form-group">
                                        <label class="input-label">
                                            Titel på materiale <span class="required">*</span>
                                        </label>
                                    ...
                                    <template v-else>
                                        <div v-if="material.type === 'video'" class="form-group">
                                            ...
                                            <div v-if="getYoutubeEmbedUrl(material.youtubeUrl)"
                                                 class="video-preview">
                                                <iframe :src="getYoutubeEmbedUrl(material.youtubeUrl)"
                                        ...
                                        <div v-if="material.type === 'pdf'" class="form-group">
                                            <label class="input-label">
                                                PDF-fil <span class="required">*</span>
                                            </label>
            ...
            <footer class="modal-footer">
                <p v-if="submitError && modules.length > 0" class="submit-error">
                    {{ submitError }}
                </p>
